...
export const processSanityPortfolioItem = getImageUrl => rawItem => {
  const {images, videos, title, year, featured} = rawItem;
  const media = (images || []).concat(videos || []).map(item => ({
    src: getImageUrl(item.imageMono) || getImageUrl(item.imageColor),
    activeSrc: getImageUrl(item.imageColor),
    videoSrc: getImageUrl(item.videoMono),
    activeVideoSrc: getImageUrl(item.videoColor)
  }));
  return {
    title,
    year,
